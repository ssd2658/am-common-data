...
public interface SecurityRepository extends BaseRepository<SecurityDocument> {
    ...
    @Query(value = "{'key.symbol': ?0}", sort = "{'audit.version': -1}")
    Optional<SecurityDocument> findBySymbol(String symbol);
    
    @Query(value = "{'key.isin': ?0}", sort = "{'audit.version': -1}")
    Optional<SecurityDocument> findByIsin(String isin);
    
    @Query(value = "{'metadata.sector': ?0}", sort = "{'audit.version': -1}")
    List<SecurityDocument> findBySector(String sector);
    
    @Query(value = "{'metadata.industry': ?0}", sort = "{'audit.version': -1}")
    List<SecurityDocument> findByIndustry(String industry);
    
    @Query(value = "{'metadata.capCategory': ?0}", sort = "{'audit.version': -1}")
    List<SecurityDocument> findByCapCategory(String capCategory);
    ...
    @Query(value = "{'metadata.exchange': ?0}", sort = "{'audit.version': -1}")
    List<SecurityDocument> findByExchange(String exchange);
    
    @Query(value = "{'metadata.tradingStatus': ?0}", sort = "{'audit.version': -1}")
    List<SecurityDocument> findByTradingStatus(String tradingStatus);
    
    @Query(value = "{'companyInfo.legalName': {$regex: ?0, $options: 'i'}}", sort = "{'audit.version': -1}")
    ...
    @Query(value = "{$and: [{'metadata.marketCapValue': {$gte: ?0}}, {'metadata.sector': ?1}, {'metadata.tradingStatus': 'ACTIVE'}]}", 
           sort = "{'metadata.marketCapValue': -1}")
